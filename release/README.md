...
The install uses npm's `--prefer-offline` mode so a cold runner can resolve transitive peer
dependencies; package lifecycle scripts remain disabled. The receipt distinguishes permitted
install-time registry access from the zero-external-request runtime check. The consumer process
removes credential-shaped environment variables and uses an empty temporary npm user config before
package import or child-process execution. The temporary project and tarballs are deleted on success
or failure. Both the general release check and the publish gate require this buyer-boundary proof.
